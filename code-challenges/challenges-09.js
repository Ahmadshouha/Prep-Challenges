...
const employeesBonus = (arr) => {
    ...
    for(let i=0;i<afterAdd.length;i++){
      if(afterAdd[i].workHours>8){
        afterAdd[i].salary =parseInt(afterAdd[i].salary.slice(0,afterAdd[i].salary.length-1))+100+"$";
      }else{
        afterAdd[i].salary =parseInt(afterAdd[i].salary.slice(0,afterAdd[i].salary.length-1))+50+"$";
      }
    }
    return afterAdd;
}
