...
const objLat = (obj) => {
    ...
    var first1 = first[0].toUpperCase();
    let long = first.length;
    first1 = first1 + first.slice(1, long);
    console.log(first1);


    var last = obj.lastName.toString();
    var last1 = last[0].toUpperCase();
    let long2 = last.length;
    last1 = last1 + last.slice(1, long2);
    console.log(last1);




    let sent = `my name is ${first1} ${last1} I am ${obj.age} YO, and I love ${obj.hobby}.`;
...
const cvFormatter = (arr) => {
    ...
    for (let i = 0; i < arr.length; i++) {
        console.log("arr[i].lastName", arr[i].lastName)
        if (arr[i].yearsOfExperience > 1) {
            ...
            arr1.push({ fullName: arr[i].firstName + arr[i].lastName, tech: arr[i].tech })
        }
    };
    return arr1;
};
...
const classesAvg = (data) => {
    ...
    for (let i = 0; i < data.grades.length; i++) {
        ...
        for (let b = 0; b < data.grades[i].classes.length; b++) {
            sum=0;
            for (let c = 0; c < data.grades[i].classes[b].classScores.length; c++) {
            
                sum= sum + data.grades[i].classes[b].classScores[c];
                
            }
            data.grades[i].classes[b].avg =Math.floor( sum / data.grades[i].classes[b].classScores.length) ;
        }    
    }
    return data;
};
